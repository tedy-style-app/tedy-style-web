...
export default function Support() {
  ...
  return (
    <div>
      <PageHeader
        title="Поддержка"
        subtitle={`Обращения из приложения · ${total.toLocaleString('ru-RU')}`}
        action={
      ...
      {state === 'ready' &&
        (rows.length === 0 ? (
          <EmptyState icon="📨" title="Обращений нет" hint="Формы из приложения будут приходить сюда." />
        ) : (
          <>
              ...
              {rows.map((f) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <Badge tone={typeTone(f.type)}>{typeLabel[f.type] || f.type}</Badge>
                  ...
                  <div className="mt-1.5 text-[12px] font-semibold text-ink-3">
                    {f.userName || 'Пользователь'}
                    {f.attachmentUrl ? ' · 📎 вложение' : ''}
                  </div>
                </button>
              ))}
            </div>
    ...
    <div className="fixed inset-0 z-[200] flex justify-end bg-black/30" onClick={onClose}>
      <div className="h-full w-full max-w-[480px] overflow-y-auto bg-page p-6 shadow-float" onClick={(e) => e.stopPropagation()}>
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-[20px] font-black text-ink">Обращение</h2>
        ...
        <div className="rounded-4xl border border-hair bg-white p-5 shadow-card">
          ...
          <div className="mt-1 text-[12px] font-semibold text-ink-3">{f.userName || 'Пользователь'}</div>
          <p className="mt-4 whitespace-pre-line text-[14px] font-medium leading-relaxed text-ink-2">{f.description}</p>
          {img && (
            <img src={img} alt="вложение" className="mt-4 w-full rounded-2xl border border-hair object-cover" />
          )}
        </div>

        <h3 className="mb-2 mt-6 text-[13px] font-extrabold uppercase tracking-wide text-ink-3">Статус</h3>
